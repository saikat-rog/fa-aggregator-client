...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-60 mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-lg bg-transparent px-3 py-3 lg:flex dark:bg-transparent",
        visible && "bg-white/80 ",
        className,
      )}
    >
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-md font-medium text-textcolor transition duration-200 hover:text-black lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-textcolor"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <button
      ...
      onClick={onClick}
      className="inline-flex items-center justify-center rounded-lg p-1 text-black"
    >
...
export const NavbarLogo = () => {
  return (
    ...
    >
      <img
        src="/assets/codehez-logo-without-text.png"
        alt="codehez-logo"
        width={40}
        height={40}
      />
      <span className="text-primary font-bold text-2xl md:text-3xl">odeHez</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-8 py-2 rounded-xl bg-white button bg-primary text-white text-md font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";
